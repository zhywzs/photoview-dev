...
- **多级缩略图**:扫描管线生成 1024/256/128px 三档(mipmap),按 `瓦片尺寸 × DPR` 自动选择,密集视图带宽约 0.8KB/张;另有**缩略图图集**(Atlas):每用户照片按时间线顺序拼入 8×8 网格的 1024×1024 JPEG 精灵图(128px/格),密集档整屏图片从 ~1800 个 HTTP 请求降为 ~30 个;扫描完成后自动重建,设置页可手动重新生成
