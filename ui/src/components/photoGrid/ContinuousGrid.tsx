...
const ContinuousGrid = <T,>({
  ...
  const handle = useMemo<ContinuousGridHandle>(
    () => ({
      ...
      setView(view) {
        ...
        if (inner != null) {
          // keep the document height tied to the *committed* layout so it
          // does not change (and drag the scroll around) mid-gesture
          const committedRows = Math.ceil(
            Math.max(1, n) / Math.max(1, rc.fromC)
          )
          const committedPitch = tileForColumns(w, rc.fromC) * (1 + r)
          inner.style.height = `${committedRows * committedPitch}px`
  ...
  // tiles are mounted at the committed level's size; the scale animates them
  const baseSize = tileForColumns(width, Math.max(1, rr.fromC))
